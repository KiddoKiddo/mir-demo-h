...
def script():
    ...
    if (input_string == 'Pause_AGV') :   #Pause the AGV
    	headers = {'Content-type': 'application/json', 'Authorization': 'Basic YWRtaW46OGM2OTc2ZTViNTQxMDQxNWJkZTkwOGJkNGRlZTE1ZGZiMTY3YTljODczZmM0YmI4YTgxZjZmMmFiNDQ4YTkxOA=='}
        #r = requests.put('http://10.100.108.210:8080/v2.0.0/state', json ={'state':3}, headers=headers)
        #r = requests.get('http://10.100.108.210:8080/v2.0.0/state', json ={'state':1}, headers=headers)
        #r = requests.get('http://10.100.108.210:8080/v2.0.0/missions/{9c4b4293-3d3c-11e8-99fb-f44d306f3f85}')
        #r = requests.post('http://10.100.108.210:8080/v2.0.0/actions', json ={"action_type_id": 300, "mission_id" : "a4c19cdc-1d34-11e8-ba33-f44d306f3f85"}, headers=headers)  #1.charginig the robot
        r = requests.get('http://10.100.108.210:8080/v2.0.0/status', json ={"whitelist" : "string"}, headers=headers )
        print(r.text)
